...
def load_and_transform_data():
    try:
        ...
        transformed_df = transformed_df[['FECHA', 'AGENTE', 'EMPRESA', 'Precio Energía USD/MWh', 'Periodo']]
...
with tab1:
    ...
    with col_right:
        ...
        df_empresa_prom = df_empresa.groupby(['FECHA', 'EMPRESA'])['Precio Energía USD/MWh'].mean().reset_index()
        precio_promedio_empresa = df_empresa['Precio Energía USD/MWh'].mean()
        ...
        fig_empresa.update_layout(yaxis_title="Precio Promedio (USD/MWh)", xaxis_title="Fecha", showlegend=False)
...
with tab2:
    ...
    df_empresas_prom_tab2 = df_filtered.groupby(['FECHA', 'EMPRESA'])['Precio Energía USD/MWh'].mean().reset_index()
